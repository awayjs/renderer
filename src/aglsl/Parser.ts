	...
	export class AGLSLParser
	{
		public parse( desc:aglsl.Description )
		{
			...
			for ( var i=0; i<desc.tokens.length; i++ ) {        
				...
				if ( !lutentry ) throw "Opcode not valid or not implemented yet: " /*+token.opcode;*/ // TODO huh? where is token coming from? Injected due to shitty JS?
		...
		public regtostring(regtype,regnum, desc:aglsl.Description, tag )
		{
			switch ( regtype ) {
				case 0x0: return "va"+regnum; 
				case 0x1: 
