...
{
	export class View3DDevReference
	{
		...
		constructor(scene:Scene3D, camera:away.cameras.Camera3D, renderer:away.render.RendererBase, forceSoftware:boolean = false, profile:string = "basline")
		{
		...
		public set width(value:number)
		{
			...
			if (this._pStageGLProxy && this._pStageGLProxy.usesSoftwareRendering && value > 2048) // TODO: Check if still valid in GLSL
			{
		...
		public pUpdateBackBuffer()
		{
			...
			if (this._pStageGLProxy._iContextGL && !this._pShareContext) {
				...
				if (this._width && this._height) {
					...
					if (this._pStageGLProxy.usesSoftwareRendering) {
