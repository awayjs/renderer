...
module away.geom
{
    ...
    export class AMatrix3D
    {
        ...
        constructor( mat : number[] = null , usefloat : boolean = false )
        {
            ...
            if( AMatrix3D.tempMul == null )
            {
                AMatrix3D.tempMul           = new Array<boolean>();
                AMatrix3D.tempMul[true]     = new AMatrix3D( null , true );
                AMatrix3D.tempMul[false]    = new AMatrix3D( null , false );

            }

            if( AMatrix3D.temp == null )
            {
                AMatrix3D.temp           = new Array<boolean>();
                AMatrix3D.temp[true]     = new AMatrix3D( null , true );
                AMatrix3D.temp[false]    = new AMatrix3D( null , false );
        ...
        public scale ( sx : number , sy : number , sz : number ) : AMatrix3D
        {
            this.identity ();
        ...
        public translation ( dx : number , dy : number , dz : number ) : AMatrix3D
        {
        ...
        {
            var cosa : number = Math.cos ( -angledeg * AMatrix3D.DEG2RAD );
            var sina : number = Math.sin ( -angledeg * AMatrix3D.DEG2RAD );
        ...
        {
            var cosa : number = Math.sin ( -angledeg * AMatrix3D.DEG2RAD );
            var sina : number = Math.cos ( -angledeg * AMatrix3D.DEG2RAD );
        ...
        public multiplyInPlace ( dest : AMatrix3D , a : AMatrix3D , b  : AMatrix3D ) : void
        {
            /*
            ...
            */
            ...
            // reference
            var orgdest : AMatrix3D ;

            if ( dest===a || dest===b )
            {
                orgdest = dest;
                dest = AMatrix3D.tempMul[dest.isfloat];
            }
        ...
        private dotRowColumn (  a : AMatrix3D , b  : AMatrix3D , ar : number , bc : number ) : number
        {
            return  a.data[ar*4  ]*b.data[bc] +
                    a.data[ar*4+1]*b.data[bc+4] +
                    a.data[ar*4+2]*b.data[bc+8] +
        ...
        public copyFrom ( othermatrix : AMatrix3D ): void
        {
        ...
        public copyTo ( othermatrix : AMatrix3D ) : void
        {
        ...
        public isEpsEqual ( other : AMatrix3D , eps : number )
        {
        ...
        public projection = function ( znear : number , zfar: number , fovdeg: number , aspect : number ) : void
        {
            ...
            var yval : number = znear * Math.tan( fovdeg * AMatrix3D.DEG2RAD * 0.5 );
