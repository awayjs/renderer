module away3d.core.math {
	...
	export class Plane3D
	{
		...
		public static ALIGN_ANY : int = 0;
		public static ALIGN_XY_AXIS : int = 1;
		public static ALIGN_YZ_AXIS : int = 2;
		public static ALIGN_XZ_AXIS : int = 3;
		...
		constructor(a : Number = 0, b : Number = 0, c : Number = 0, d : Number = 0)
		{
			...
			this.c = c;
			this.d = d;
			if (a == 0 && b == 0) _alignment = Plane3D.ALIGN_XY_AXIS;
			else if (b == 0 && c == 0) _alignment = Plane3D.ALIGN_YZ_AXIS;
			else if (a == 0 && c == 0) _alignment = Plane3D.ALIGN_XZ_AXIS;
			else _alignment = Plane3D.ALIGN_ANY;
		}
		...
		{
			if (_alignment == Plane3D.ALIGN_YZ_AXIS)
				return a*p.x - d;
			else if (_alignment == Plane3D.ALIGN_XZ_AXIS)
				return b*p.y - d;
			else if (_alignment == Plane3D.ALIGN_XY_AXIS)
				return c*p.z - d;
            else
		 ...
		 */
		public classifyPoint(p : Vector3D, epsilon : Number = 0.01) : int
		{
			// check NaN
