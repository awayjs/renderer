...
export class ShaderBase implements IAbstractionPool
{
	...
	public _setRenderState(renderState:_Render_RenderableBase, projection:ProjectionBase):void
	{
		...
		if (this.usesUVTransform) {
			var uvMatrix:Matrix = renderState.uvMatrix;
		...
		if (this.usesColorTransform) {
			...
			var colorTransform:ColorTransform = renderState.sourceEntity._iAssignedColorTransform();
	...
	private _updateProgram():void
	{
		...
		var programData:ProgramData = this._stage.getProgramData(this._animationVertexCode + this._vertexCode, this._fragmentCode + this._animationFragmentCode + this._postAnimationFragmentCode);
    ...
    private _compileAnimationCode():void
    {
        ...
        if (this.usesAnimation) {
            ...
            var len:number = this._sharedRegisters.animatableAttributes.length;
